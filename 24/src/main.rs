...
impl Group {
    fn new(
        ...
        attack_damage: usize,
        attack_type: DamageType,
        weaknesses_list: Vec<DamageType>,
        immunities_list: Vec<DamageType>,
        initiative: usize,
        team: Team,
    ) -> Self {
...
fn fight(mut groups: Vec<Group>, part_two: bool) -> (bool, usize) {
    loop {
        ...
        for group in &groups {
            ...
            for target in &groups {
                if target.id == group.id
                    || target.team == group.team
                    || target.immunities.contains(&group.attack_type)
                    || chosen_targets.contains(&target.id)
                {
                    continue;
                }
        ...
        for attacker in &attacking_ids {
            ...
            let attacker: Group = groups
                .iter()
                .filter(|g| g.id == *attacker)
                .nth(0)
                .unwrap()
                .to_owned();
            ...
            let target: &mut Group = groups
                .iter_mut()
                .filter(|g| g.id == *target)
                .nth(0)
                .unwrap();
...
fn main() {
    ...
    let mut groups = vec![
        Group::new(
            ...
            17,
            DamageType::Cold,
            vec![DamageType::Cold, DamageType::Bludgeoning],
        ...
        Group::new(
            ...
            71,
            DamageType::Fire,
            vec![DamageType::Bludgeoning, DamageType::Radiation],
            vec![],
            18,
            Team::Infection,
        ),
        ...
        Group::new(
            ...
            73,
            DamageType::Cold,
            vec![DamageType::Radiation, DamageType::Bludgeoning],
            vec![],
            3,
            Team::Infection,
        ),
